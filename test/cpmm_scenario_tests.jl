@testitem "scenario 1" begin
    ...
    yes = true
    midx = 1
    price1 = compute_price(market, midx, yes)

    # buy n_shares 
    cost = shares_to_cost(market, n_shares, midx, yes)
    yes_reserves1, no_reserves1 = update_reserves!(market, n_shares, cost, midx, yes)
    money -= cost
    price2 = compute_price(market, midx, yes)
    @test price2 > price1
    @test yes_reserves * no_reserves ≈ yes_reserves1 * no_reserves1

    # sell n_shares 
    cost = shares_to_cost(market, -n_shares, midx, yes)
    yes_reserves2, no_reserves2 = update_reserves!(market, -n_shares, cost, midx, yes)
    money -= cost
    price3 = compute_price(market, midx, yes)
    @test price1 ≈ price3
